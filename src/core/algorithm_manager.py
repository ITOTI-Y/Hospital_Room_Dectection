...
class AlgorithmManager:
    ...
    def run_single_algorithm(
        ...
        if algorithm_name not in self.algorithm_registry:
            raise ValueError(
                f"未知算法: {algorithm_name}. 可用算法: {list(self.algorithm_registry.keys())}"
            )
    ...
    def _run_algorithms_parallel(
        ...
        def run_algorithm_independent(
            alg_name: str,
            init_layout: Optional[List[str]],
            params: Dict[str, Any],
            orig_layout: List[str],
            orig_cost: float,
        ):
            """在独立的上下文中运行算法"""
            # 创建独立的优化器实例
            optimizer = self._create_optimizer(
                alg_name, params, create_independent=True
            )
        ...
        if algorithm_name == "ppo":
            ...
            if custom_params:
                if "initial_temperature" in custom_params:
                    sa_params["initial_temperature"] = custom_params[
                        "initial_temperature"
                    ]
                ...
                if "temperature_length" in custom_params:
                    sa_params["temperature_length"] = custom_params[
                        "temperature_length"
                    ]
    ...
    def save_results(self, output_dir: str = "./results/comparison"):
        ...
        for algorithm_name, result in self.results.items():
            ...
            if result.original_cost is not None and result.original_cost > 0:
                result_dict["improvement"] = (
                    (result.original_cost - result.best_cost) / result.original_cost
                ) * 100
    ...
    def register_algorithm(
        self, name: str, optimizer_class: Type[BaseOptimizer], config: Dict[str, Any]
    ):
        """
        注册新算法
