...
class PeopleFlow:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            and self.actual_node_id_sequence == other.actual_node_id_sequence
        )
    ...
    def __repr__(self) -> str:
        ...
        """
        flow_str = (
            " -> ".join(self.actual_node_id_sequence)
            if self.actual_node_id_sequence
            else "Empty"
        )
        return (
...
class PathFinder:
    ...
    def __init__(
        self, csv_filepath: str | None = None, config: NetworkConfig | None = None
    ):
        """
        ...
        if csv_filepath:
            ...
            self.csv_filepath: Path = (
                self.config.RESULT_PATH / "super_network_travel_times.csv"
            )
    ...
    def _load_and_process_csv(self) -> None:
        ...
        if (
            df.index.name is not None or df.index.dtype == "object"
    ...
    def generate_flows(
        ...
        custom_assignment_map: Optional[Mapping[str, List[str]]] = None,
    ) -> List[PeopleFlow]:
        ...
        assignment_to_use = (
            custom_assignment_map
            if custom_assignment_map is not None
            else self.name_to_ids_map
        )
        ...
        for id_sequence_tuple in all_possible_id_sequences:
            ...
            current_flow_id = (
                f"{workflow_identifier}_{flow_counter_for_identifier}"
                if isinstance(workflow_identifier, str)
                else (workflow_identifier, flow_counter_for_identifier)
            )

            flow = PeopleFlow(
                identify=current_flow_id,
    ...
    def get_travel_time(self, from_name_id: str, to_name_id: str) -> Optional[float]:
        ...
        try:
            ...
            if (
                from_name_id in self.travel_times_df.index
                and to_name_id in self.travel_times_df.columns
            ):
