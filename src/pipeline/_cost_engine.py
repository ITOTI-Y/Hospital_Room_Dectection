        ...
        for pathway in self.pathways:
            ...
            if start_nodes:
                ...
                for start_node in start_nodes:
                    pair = (
                        ...
                        if start_node < sequence[0]
                        else (sequence[0], start_node)
                    )
            ...
            if end_nodes:
                ...
                for end_node in end_nodes:
                    pair = (
                        (sequence[-1], end_node)
                        if sequence[-1] < end_node
                        else (end_node, sequence[-1])
                    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        )  # 这个由config_loader处理，暂时保留硬编码

        if (
            not travel_times_path.exists()
            or not graph_config_path.exists()
        ...
        service_times = {
            name: data.get("service_time", 0) for name, data in node_defs.items()
        }
        ...
        sorted_weights = sorted(
            engine.pair_weights.items(), key=lambda item: item[1], reverse=True
        )
        ...
        if len(depts_to_layout) >= 2:
            ...
            if slot1 and slot2:
                delta = engine.calculate_swap_delta_cost(
                    dept1, dept2, slot1, slot2, initial_layout
                )
                logger.info(
                    f"随机交换 '{dept1}' 和 '{dept2}' 的成本变化 (Delta): {delta:.2f}"
                )
        else:
