...
class NetworkGenerator:
    ...
    def generate_network(
        ...
        if not image_dir.is_dir():
            logger.error(
                f'Floor annotation images directory does not exist: {image_dir}'
            )
            return False

        image_file_paths = [p for p in sorted(image_dir.glob('*.png')) if p.is_file()]
    ...
    def run_complete_generation(
        ...
        if self.super_graph:
            ...
            logger.info(
                f'Saving generated network graph to {graph_path / "hospital_network.pkl"}'
            )
