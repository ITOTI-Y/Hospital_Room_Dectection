...
class AutoregressiveActor(nn.Module):
    ...
    def forward(
        ...
        dept1_embeddings = node_embeddings.gather(
            1, action1.unsqueeze(-1).unsqueeze(-1).expand(-1, -
                                                          1, self.node_hidden_dim)
        ...
        logits2: torch.Tensor = self.second_action_head(
            combined_embeddings).squeeze(-1)  # (batch_size, num_nodes)
