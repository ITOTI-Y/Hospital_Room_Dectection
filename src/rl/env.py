...
class LayoutEnv(gym.Env):
    ...
    def __init__(self, config: ConfigLoader, max_departments: int, max_step: int):
        ...
        self.numerical_feature_dim = 6 # [service_time, service_weight, area, x, y, z]
        ...
        self.observation_space = spaces.Dict({
            "x_numerical": spaces.Box(low=-5, high=5, shape=(self.max_departments, self.numerical_feature_dim), dtype=np.float32),
            "x_categorical": spaces.Box(low=0, high=self.num_total_slot - 1, shape=(self.max_departments,), dtype=np.int32),
            "edge_index": spaces.Box(low=0, high=self.max_departments - 1, shape=(2, self.E_max), dtype=np.int32),
            "edge_weight": spaces.Box(low=0, high=1, shape=(self.E_max,), dtype=np.float32),
            "node_mask": spaces.MultiBinary(self.max_departments),
            "edge_mask": spaces.MultiBinary(self.E_max),
        })
        self.action_space = spaces.MultiDiscrete([self.max_departments, self.max_departments])
    ...
    def _precompute_normalization_stats(self) -> None:
        ...
        features = self.cost_manager.slots[['service_time', 'service_weight', 'area', 'pos_x', 'pos_y', 'pos_z']]
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[Dict] = None):
        ...
        self.logger.info(f"New instance created. Active departments: {self.num_total_slot}, Initial travel cost: {self.initial_cost}")
    ...
    def _get_observation(self):
        ...
        x_numerical = np.zeros((self.max_departments, self.numerical_feature_dim), dtype=np.float32)
        x_categorical = np.zeros((self.max_departments,), dtype=np.int32)
        node_mask = np.zeros((self.max_departments,), dtype=np.int32)

        x_norm_numerical = self.scaler.transform(self.cost_manager.slots[['service_time', 'service_weight', 'area', 'pos_x', 'pos_y', 'pos_z']])
        x_numerical[:self.num_total_slot, :] = x_norm_numerical

        x_categorical[:self.num_total_slot] = np.array([i for i in self.index_to_dept.keys()])
