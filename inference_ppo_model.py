...
class PPOModelInference:
    ...
    def infer_single_episode(self, render: bool = False) -> Dict[str, Any]:
        ...
        env = ActionMasker(
            LayoutEnv(**self.env_kwargs), 
            LayoutEnv._action_mask_fn
        )
        ...
        while not terminated:
            # 获取当前的动作掩码
            action_mask = env.env.get_action_mask() if hasattr(env, 'env') else env.get_action_mask()
        ...
        logger.info(f"  面积匹配度 - 平均: {avg_area_match:.3f}, 最小: {min_area_match:.3f}, 最大: {max_area_match:.3f}")
        
        # 在verbose模式下显示面积不匹配的详细信息
        if render and area_mismatches:
            logger.info("\n  面积不匹配的分配（匹配度<0.8）:")
            for mismatch in area_mismatches[:5]:  # 只显示前5个
                logger.info(f"    {mismatch['department']:20s} -> {mismatch['slot']:20s}: "
    ...
    def infer_multiple_episodes(self, n_episodes: int = 5) -> Dict[str, Any]:
        ...
        logger.info(f"  平均面积匹配度: {stats['statistics']['mean_area_match']:.3f} (±{stats['statistics']['std_area_match']:.3f})")
        logger.info("="*60)
            ...
            with open(layout_path, 'w', encoding='utf-8') as f:
                ...
                if 'area_match_stats' in results['best_result'] and stats['mismatches']:
                    ...
                    for mismatch in stats['mismatches']:
                        f.write(f"{mismatch['department']:25s} -> {mismatch['slot']:25s}: "
